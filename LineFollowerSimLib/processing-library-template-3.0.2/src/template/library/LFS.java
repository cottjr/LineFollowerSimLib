 ...
 * One instance should be created of LFS in your program. For example in setup() method include the line:
 * <p>
 * lfs = new LFS(this);
 * 
 */
...
public void drawRobotAndCourseViews(int rvDiv, int cvDiv, boolean cRotate90) // divisors
{
  ...
  {	
	...
	if (--rvCount<= 0)
	{ rvCount = rvDiv;	
      view.drawRobotView(course,courseDPI,robot);   // draw robot view into defined viewport
	}  
  }
  ...
  {	
	...
	if (--cvCount<=0)
	{ cvCount = cvDiv;
	  view.drawCourseView(course,robot,courseDPI,cRotate90, contestState == ContestStates.csFinished);  // contest Finished draw (F) on course 
	}  
  } 
 ...
 * @return X (inches) 
 */
public float getRobotX () {  if (!contestIsRunning()) return robot.x;  else { PApplet.println ("Contest Running robot location not available"); return 0; }}
/**
 * If contest is not running return robot center, coordinate location Y value (inches)
 * @return Y (inches) 
 */
public float getRobotY () {  if (!contestIsRunning()) return robot.y;  else { PApplet.println ("Contest Running robot location not available"); return 0; }}
/**
